...
def process_file():
	...
	df_blocks.drop(columns=['Unnamed: 0', 'Unnamed: 1', 'Unnamed: 2', 'Unnamed: 3', 
					 'Unnamed: 4', 'Unnamed: 5', 'Unnamed: 6'], inplace=True)

	df_districts.drop(columns=['Unnamed: 0', 'Unnamed: 1', 'Unnamed: 2', 'Unnamed: 3', 
					 'Unnamed: 4', 'Unnamed: 5', 'Unnamed: 6'], inplace=True)
...
def reformat_months(df, dftype):
	...
	df = df[['Name_{}'.format(dftype), 'Individual_UID', 'Designation']]

	df['Designation_{}'.format(dftype)] = df['Designation'].apply(lambda x: "_".join(x.split("_", 2)[2:]) if isinstance(x, str) else '')

	df['Location_{}'.format(dftype)] = df['Designation'].apply(lambda x: "_".join(x.split("_", 2)[:2]) if isinstance(x, str) else '')
